...
class SimpleCaptcha {
   ...
   // the text that you need to watermark		Updated: 2013-03-22 by Dave Henderson (dhenderson@cliquesoft.org)
   public $watermark = "domain.com";
...
//            if ($this->watermark) {		// Updated: 2013-03-22 by Dave Henderson (dhenderson@cliquesoft.org)
//		$tb = imagettfbbox($fontcfg['minSize'], 0, $fontfile, $this->watermark);	// taken from http://php.net/manual/en/function.imagettftext.php
// LEFT OFF - fix the height value below to match the width, but using the right $tb[] value
//                $coords = imagettftext($this->im, $fontcfg['minSize'], 0,
//                    ceil(($this->width-$tb[2])/2), ceil(($this->height-$tb[])/2),
